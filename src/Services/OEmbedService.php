...
*/
namespace Tall\Editor\Services;

use Cache;
use Http;
use Illuminate\Support\Arr;
use Tall\Editor\Exceptions\OEmbedFetchException;
...
class OEmbedService
{
    ...
    {
        $cache = config('editor.embed.cache.enabled', true);
        $key = "editor_embed_parse_{$url}";
        ...
        if ($cache) {
            Cache::set($key, $data, config('editor.embed.cache.duration', 3600));
        }
    ...
    {
        $maxwidth = intval(config('editor.embed.maxwidth', 1000));
        $maxheight = intval(config('editor.embed.maxheight', min($maxwidth * 1.5, 1000)));
        $dnt = 1;
